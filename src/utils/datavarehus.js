...
export var runIntersection = async() => {
    if(currentMapFeature){
        ...
        {  
            removeMapFeatureIfExists();
            L.popup().setLatLng(map.getCenter()).setContent('No hit for dataset on selected area').openOn(map);
        }   
    }else{
        removeMapFeatureIfExists();
        L.popup().setLatLng(map.getCenter()).setContent('No area selected for intersection query').openOn(map);
    }
}
window.runIntersection = runIntersection; //Set function as globally accessible